...
\paragraph{GraphQL Server response structure & error handling}
...
\begin{itemize}
  \item \textbf{Data}: il campo data viene ritornato nella risposta solo qualora la query abbia avuto successo anche solo parzialmente e vengono dunque ritornati al suo interno i dati richiesti;
...
\begin{itemize}
  ...
  \item \textbf{Errori nei campi}: si verificano quando vengono specificati nella richiesta tipi o campi non presenti. In questo caso l'errore si verifica durante l'esecuzione, dunque a volte può essere ritornato sia il campo \textit{data}, contenente i dati che il server è riuscito a recuperare, che il campo \textit{errors} che indica la presenza di uno o più errrori di data fetching;
  \item \textbf{Errori di }
\end{itemize}
...
\end{figure}
\FloatBarrier
Attraverso il metodo \textit{watchQuery<>} dell'oggetto \textit{apollo} è possibile eseguire la query all'endopoint del server GraphQL. Il metodo ritorna un oggetto di tipo \textit{Observable}. La risposta che viene ricevuta dal client dopo la richiesta contiene un JSON con campi \textit{"data"} se la richiesta è andata a buon fine, altrimenti ritorna un campo \textit{"errors"} contenente l'errore, tuttavia possiamo trovarci in due casi:
\begin{itemize}
  \item \textbf{Errore nella formulazione della query}: i
\end{itemize}
