...
\subsubsection*{Implementazione FE}
...
\end{table}
\begin{itemize}
  \item \textbf{UtenteController}: espone le API per la gestione degli utenti;
  \item \textbf{TavoloController}: espone le API per la gestione delle sessioni nei tavoli e dunque degli ordini;
  \item \textbf{MenuController}: espone le API per la visualizzazione dei menù
  \item \textbf{LoginController}: espone le API per il login;
  \item \textbf{GestoreController}: espone le API per tutte quelle funzionalità disponibili al gestore;
\end{itemize}
