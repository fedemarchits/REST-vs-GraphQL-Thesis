...
\end{itemize}
\textcolor{red}{Parlare della gestione delle eccezioni.........}
\paragraph{Testing API}
Essendo un prototipo incentrato sulla realizzazione delle API, sono stati svolti in maniera semplice e veloce i test per gli strati di servizio e repository, mentre sono stati svolti dei test più approfonditi per quanto riguarda lo strato di controller.\\
Per eseguire i test sui metodi del controller è stato scelto di utilizzare il framework JUnit5 \textcolor{red}{(RINDIRIZZARE AL CAPITOLO TECNOLOGIE)} ed è stato utilizzato il classico pattern \textit{Arrange - Act - Assert} per la realizzazione di ciascun test.\\
Prima di procedere visualizzando i test bisogna premettere che è stato utilizzato un database identico, ma con dati differenti al fine di poter testare quanto risposto dai metodi del controller. \\
È stata creata una una classe per ciascun controller al fine di testarne tutti i metodi. Nell'immagine \ref{EmployeeControllerTest} l'esempio di un test \textit{listEmployeeByNameTest} della classe di test \textit{EmployeeControllerTest}:
\FloatBarrier
\begin{figure}[!ht]
\begin{mdframed}
\centering
\includegraphics[width=1\linewidth]{immagini/EmployeeControllerTest.png}
\end{mdframed}
