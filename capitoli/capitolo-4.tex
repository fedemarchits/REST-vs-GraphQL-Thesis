...
Il primo test realizzato è uno \textit{SmokeTest} e da qui il nome della classe realizzata per testare che il contesto Spring abbia creato i controller dell'applicazione. In figura \ref{smoke-test} la classe \textit{SmokeTest} con una dipendenza per ciascun controller risolta con la dependency injection grazie all'annotazione \textbf{@Autowired}. Infine un metodo per la verifica sulla presenza o meno di ciascun controller.
\FloatBarrier
\begin{figure}[!ht]
\begin{mdframed}
\centering
\includegraphics[width=1\linewidth]{immagini/SmokeTest.png}
\end{mdframed}
\caption{Classe \textit{SmokeTest} sulla creazione dei controller.}
\label{smoke-test}
\end{figure}
\FloatBarrier
Passiamo ora ai test sui singoli metodi dei controller. Nell'immagine \ref{EmployeeControllerTest} l'esempio di un test \textit{listEmployeeByNameTest} della classe di test \textit{EmployeeControllerTest}:
\FloatBarrier
\begin{figure}[!ht]
\begin{mdframed}
\centering
